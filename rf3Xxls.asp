...
<fieldset>
<LEGEND><span class="style3">一卡通通借狀況表</span></LEGEND>
<% 
Response.AddHeader "Content-Disposition","attachment;filename=rf3X.xls" 
Response.ContentType = "application/vnd.ms-excel" 
%> 
<!--form action="rf3X.asp" method="POST">
  ...
  if not rs_list.eof then    
   ...
   %>
    ...
    <!--table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("rf3X.asp",kword,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
               <input name="btn_upd")%>" type="button" onclick="MM_goToURL('self','rf3Xxls.asp?chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>');return document.MM_returnValue"  value="EXCEL輸出" /></span>             
