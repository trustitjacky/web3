...
function   checkall(){   
  var checkitem=document.all.choice;
  if (checkitem) {
     for (var i=0; i<checkitem.length; i++) {
        checkitem[i].checked=(event.srcElement.checked)?true:false;
     }
  }
} 

function allchecks(){ 
  var   obj=document.all.choice;
 if (obj) { 
  for(i=0;i<obj.length;i++) { 
     if (obj[i].checked==true) {             
        MM_goToURL('self','disbution2X-db.asp?id='+obj[i].value+'&search_e_title=<%=SESSION("search_e_title")%>&kword=<%=Server.UrlEncode(kword)%>&PID=<%=PID%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&myPageSize=<%=myPageSize%>&page=<%=page%>&epage=<%=epage%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>'); 
        obj[i].checked=false;
        alert('確認配送')
...
<fieldset>
<LEGEND><span class="style3">一卡通集中地配送作業</span></LEGEND>
<form action="disbution2X.asp" method="POST" name="form1">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <% 
          ...
          <%     end if
                 rs_dep.CLOSE                 
          %> 
	  <% if  rA_right="1" or GroupID="1" then
	         sqlstr_dep = "SELECT UserName FROM department where 1=1"
                 SET rs_dep = Server.CreateObject("ADODB.Recordset")
	         rs_dep.OPEN sqlstr_dep, adoConn, 3,3
                 if not rs_dep.eof then %>
                    ...
                    <select name="Target_dep" id="Target_dep">
                        <option value="">=請選擇=</option>            
 	       	     <% FOR SH=1 to rs_dep.recordcount %>                
 	       	       <% if   trim(rs_dep("UserName")) <> "基隆市文化局圖書館" then %>         
                        <option value="<%=trim(rs_dep("UserName"))%>"><%=Trim(rs_dep("UserName"))%></option>
                       <% end if %>
                     <% rs_dep.MOVENEXT                                
                        IF rs_dep.EOF THEN EXIT FOR 
   ...
   <!--<table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
      <tr>
        <td colspan="2" bgcolor="#FFFFFF"><div align="right">
            <input name="Upload" type="submit" onclick="window.open('upload/upfile2X.asp','fileup','width=530,height=50')" value="檔案上傳" />
        </div></td>
      </tr>
    </table>-->
      <table width="90%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="e3e3e3">
        <tr>
          <td width="6%" bgcolor="#D0DFFF"><div align="center" class="style3">
            <div align="center">
            <input name="choiceAll" type="checkbox" onclick="checkall();"/></div>            
          </div></td>
          ...
          <td width="12%" bgcolor="#D0DFFF"><div align="center" class="style3">
            <div align="center">配送日期</div>
          </div></td>
          <td width="6%" bgcolor="#D0DFFF"><div align="center" class="style3">
            <div align="center">狀態</div></td>
          <td width="8%" bgcolor="#D0DFFF"><div align="center" class="style3">
            <div align="center">配送人</div></td>            
          <td width="6%" bgcolor="#D0DFFF"><div align="center" class="style3">
            <div align="center">維護</div></td>  
        </tr>
  <%
  sqlstr_list = "SELECT P_id,P_NO,P_Name,Source_id,Target_id,Deliver_User,Send_Date,Deliver_Date,Status FROM publishesX where 1=1 "
  if len(kword)>0 then
  ...
  if not rs_list.eof then    
   ...
   %>
      <% if Trim(rs_list("Target_id")) = "基隆市文化局圖書館" then %>  
        <tr>
          <% 'if rs_list("Status")=1 then %>
          <!--td bgcolor="#FFFFFF"><span class="style3"><input type="checkbox" name="choice" id="choice_<%=rs_list("P_id")%>" value="<%=rs_list("P_id")%>" /></span></td-->
          <% 'else %>
          <td bgcolor="#FFFFFF"><span class="style3"></span></td>
          <% 'end if %>
          ...
          <td bgcolor="#FFFFFF"><span class="style3">
          <% if rs_list("Status")=x then %>
          <input name="btn_upd<%=rs_list("P_id")%>" type="button" onclick="MM_goToURL('self','disbution2X-db.asp?id=<%=rs_list("P_id")%>&search_e_title=<%=SESSION("search_e_title")%>&kword=<%=Server.UrlEncode(kword)%>&PID=<%=PID%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&myPageSize=<%=myPageSize%>&page=<%=page%>&epage=<%=epage%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>');return document.MM_returnValue"  value="配送" /></span></td>
          <% end if %>
        </tr>
      <% else %>
        <tr>
    ...
    <table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        <tr><td colspan="2" bgcolor="#FFFFFF" class="style3"><div align="left">
        <input type="button" value="批次配送" onclick="allchecks();" />        
        </div><td>
        </tr>
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("disbution2X.asp",kword,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>   
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
