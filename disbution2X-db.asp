...
'''''Database''''''''''''''''
        sql = "update publishesX set "
	sql = sql & "Deliver_Date = '" & now() & "', "
	sql = sql & "Deliver_User = '" & session("username_login") & "', "
	sql = sql & "Status = 2 "
	sql = sql & "where P_id = " & id & ""
        adoconn.execute(sql)        
        'record
        sqlstr_list = "SELECT * FROM publishesX where P_id = " & id & ""
        ...
        if NOT rs_list.eof then
           ...
           rStatus=2
           if len(rs_list("Center_Date"))>0 then
              rCenter_Date=rs_list("Center_Date")
           else
              rCenter_Date=" "
           end if            
           rDeliver_Date=now()
           if len(rs_list("Center_User"))>0 then
              rCenter_User=rs_list("Center_User")
           else
              rCenter_User=" "
           end if   
           rDeliver_User=session("username_login")
           sql_r = "insert into recordX(P_NO,P_Name,Source_id,Target_id,Update_User,Send_Date,Recive_Date,Status,Center_Date,Deliver_Date,Center_User,Deliver_User)values('"&rP_NO&"','"&rP_Name&"','"&rSource_id&"','"&rTarget_id&"','"&rUpdate_User&"','"&rSend_Date&"','"&rRecive_Date&"','"&rStatus&"','"&rCenter_Date&"','"&rDeliver_Date&"','"&rCenter_User&"','"&rDeliver_User&"')" 
           adoConn.execute(sql_r)  
        end if 
        rs_list.CLOSE
        response.Redirect "disbution2X.asp"&"?search_e_title="&SESSION("search_e_title")&"&kword="&Server.UrlEncode(kword)&"&PID="&PID&"&chkStatus="&chkStatus&"&ordby="&ordby&"&myPageSize="&myPageSize&"&page="&page&"&epage="&epage&"&Source_dep="&Server.UrlEncode(Source_dep)&"&Target_dep="&Server.UrlEncode(Target_dep)
%>
