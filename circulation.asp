 ...
 if (obj) { 
  for(i=0;i<obj.length;i++) { 
     if (obj[i].checked==true) {             
        MM_goToURL('self','circulation-db.asp?id='+obj[i].value+'&search_e_title=<%=SESSION("search_e_title")%>&kword=<%=Server.UrlEncode(kword)%>&PID=<%=PID%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&myPageSize=<%=myPageSize%>&page=<%=page%>&epage=<%=epage%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>'); 
        obj[i].checked=false;
        alert('確認點收')
...
<fieldset>
<LEGEND><span class="style3">一卡通通還作業</span></LEGEND>
<form action="circulation.asp" method="POST" name="form1">
  ...
  if not rs_list.eof then    
   ...
   %>
    ...
    <table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("circulation.asp",kword,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>   
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
