 ...
 if (obj) { 
  for(i=0;i<obj.length;i++) { 
     if (obj[i].checked==true) {             
        MM_goToURL('self','circulationX-db.asp?id='+obj[i].value+'&search_e_title=<%=SESSION("search_e_title")%>&kword=<%=Server.UrlEncode(kword)%>&PID=<%=PID%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&myPageSize=<%=myPageSize%>&page=<%=page%>&epage=<%=epage%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>'); 
        obj[i].checked=false;
        alert('確認點收')
...
<fieldset>
<LEGEND><span class="style3">一卡通通借作業</span></LEGEND>
<form action="circulationX.asp" method="POST" name="form1">
  ...
  <%
  sqlstr_list = "SELECT P_id,P_NO,P_Name,Source_id,Target_id,Update_User,Send_Date,Recive_Date,Status FROM publishesX where 1=1 "
  if len(kword)>0 then
  ...
  if not rs_list.eof then    
   ...
   %>
    ...
    <table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("circulationX.asp",kword,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>   
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
