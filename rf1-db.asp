...
if rA_right="1" then   
    ...
    IF len(StartDate) > 0 and len(EndDate)>0 THEN
        sql = "DELETE FROM record where Send_Date between #"&Startdate&"# and #"&Enddate&"#"
        adoconn.execute(sql)    
        response.Redirect "rf1.asp"
    ELSE 
         response.write("<script>alert('日期區間未設定，請重設！');location.href='rf1.asp';</script>")
    END IF  
end if 
%>