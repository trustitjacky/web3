...
<script type="text/javascript">
  ...
  if not rs_list.eof then    
   ...
   %>
    ...
    <table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("rf3X.asp",StartDate,EndDate,kword,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
            計<input name="text" type="text" size="8" readonly value="<%=rs_list.recordcount%>" />筆  
             <input name="btn_upd" type="button" onclick="MM_goToURL('self','rf3Xxls.asp?StartDate=<%=StartDate%>&EndDate=<%=EndDate%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>');return document.MM_returnValue"  value="EXCEL輸出" /></span>         
