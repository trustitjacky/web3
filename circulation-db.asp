...
'''''Database''''''''''''''''
	...
	sql = sql & "where P_id = " & id & ""
        ...
        rs_list.CLOSE                
        response.Redirect "circulation.asp"&"?search_e_title="&SESSION("search_e_title")&"&kword="&Server.UrlEncode(kword)&"&PID="&PID&"&chkStatus="&chkStatus&"&ordby="&ordby&"&myPageSize="&myPageSize&"&page="&page&"&epage="&epage&"&Source_dep="&Server.UrlEncode(Source_dep)&"&Target_dep="&Server.UrlEncode(Target_dep)
%>
