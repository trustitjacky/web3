...
'''''Database''''''''''''''''
	...
	sql = sql & "Recive_Date = '" & now() & "', "
	sql = sql & "Update_User = '" & session("username_login") & "', "
	sql = sql & "Status = 3 "
	sql = sql & "where P_id = " & id & ""
        ...
        if NOT rs_list.eof then
           ...
           rUpdate_User=session("username_login")
        ...
        rs_list.CLOSE                
        response.Redirect "circulationX.asp"&"?search_e_title="&SESSION("search_e_title")&"&kword="&Server.UrlEncode(kword)&"&PID="&PID&"&chkStatus="&chkStatus&"&ordby="&ordby&"&myPageSize="&myPageSize&"&page="&page&"&epage="&epage&"&Source_dep="&Server.UrlEncode(Source_dep)&"&Target_dep="&Server.UrlEncode(Target_dep)
%>
