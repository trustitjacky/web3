...
<fieldset>
<LEGEND><span class="style3">一卡通通借狀況Excel表</span></LEGEND>
<% 
Response.AddHeader "Content-Disposition","attachment;filename=rf1X.xls" 
Response.ContentType = "application/vnd.ms-excel" 
%> 
<!--form action="rf1Xxls.asp" method="POST">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr><td class="style3"> 
          移送日期<input name="StartDate" type="text" id="StartDate" value="<%=StartDate%>" />~ 
          <input name="EndDate" type="text" id="EndDate"  value="<%=EndDate%>" />(yyyy/mm/dd)	
<script type="text/javascript">
    Calendar.setup({
    ...
    Calendar.setup({
	  ...
	  <% if  rA_right="1" then
          ...
          %> 	               
          <input type="submit" name="Submit4" value="搜尋" />
          │排序依據
          <select name="ordby">
          ...
          <input type="submit" name="Submit5" value="排序" /></td>
      </tr>
     </table>
</form><br /-->  
  ...
  <%
  'sqlstr_list = "SELECT P_id,P_NO,P_Name,Source_id,Target_id,Update_User,Send_Date,Recive_Date,Status FROM recordX where 1=1 "
  sqlstr_list = "SELECT * FROM recordX where 1=1 "
  'if len(kword)>0 then
  ...
  if not rs_list.eof then    
   ...
   %>
    ...
    <!--table width="90%" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#FFFFFF">
        <tr>
          <td colspan="2" bgcolor="#FFFFFF" class="style3"><%  CALL page_change_10("rf1Xxls.asp",StartDate,EndDate,word,PID,chkStatus,ordby,myPageSize,Source_dep,Target_dep) %></td>
          <td colspan="2" bgcolor="#FFFFFF"><div align="right">
             計<input name="text" type="text" size="8" readonly value="<%=rs_list.recordcount%>" />筆  
             <input name="btn_del" type="button" onclick="javascript:if (confirm('確認刪除')) MM_goToURL('self','rf1X-db.asp?StartDate=<%=StartDate%>&EndDate=<%=EndDate%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>');return document.MM_returnValue"  value="依日期區間刪除" /></span>                      
             <input name="btn_upd" type="button" onclick="MM_goToURL('self','rf1Xxls.asp?StartDate=<%=StartDate%>&EndDate=<%=EndDate%>&chkStatus=<%=chkStatus%>&ordby=<%=ordby%>&Source_dep=<%=Server.UrlEncode(Source_dep)%>&Target_dep=<%=Server.UrlEncode(Target_dep)%>');return document.MM_returnValue"  value="EXCEL輸出" /></span>         
